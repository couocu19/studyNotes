...
​             2.实例变量：存放类的属性数据信息，包括父类的属性信息，如果是数组的实例部分还包括数组的长度，   这部分按照4字节对齐。
...
​     这是Mark Word的结构也变成了偏向锁的结构，当这个线程再次请求锁时，无需再做任何同步操作，即省去了获取锁的过程，这样就省去了大量有关锁申请的操作，从而也就提高了程序的性能。**所以，对于没有锁竞争的场合，偏向锁有很好的优化效果。**
