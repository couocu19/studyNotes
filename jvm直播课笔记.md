...
​      答：**局部变量表，操作数栈，动态链接，方法出口。**
  ...
  **操作数栈： **作为程序运行过程中操作数临时做内存中转的内存空间。

**（首先将局部变量的值压入栈顶，然后将其取出赋值给局部变量表中对应的内存对应的变量**。

​    执行计算：将栈顶的元素取出进行运算，运算结果再次压入操作数栈）。     
...
​           为什么要引入程序计数器？答：jvm支持多线程，当有多个线程在jvm分配内存，之前的线程可能会被挂起，这时需要执行当前线程，当执行完比之后要回到被挂起的线程对应的内存地址。
